  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64 flex-col">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-pizza-orange mb-4"></div>
        <p className="text-pizza-brown">Caricamento prodotti...</p>
      </div>
    );
  }

  return (
    <section id="products" className="py-24 bg-gradient-to-br from-cream-50 to-cream-100 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-10 left-10 text-pizza-orange">
          <Pizza size={120} />
        </div>
        <div className="absolute top-32 right-20 text-pizza-red">
          <ChefHat size={80} />
        </div>
        <div className="absolute bottom-20 left-1/4 text-pizza-orange">
          <Sparkles size={60} />
        </div>
      </div>
