...
function testUseCallbackUsage() {
  ...
  const useCustomerAuthPath = path.join(__dirname, '../hooks/useCustomerAuth.tsx');
  const useUserOrdersPath = path.join(__dirname, '../hooks/useUserOrders.tsx');
  
  try {
    const authContent = fs.readFileSync(useCustomerAuthPath, 'utf8');
    const ordersContent = fs.readFileSync(useUserOrdersPath, 'utf8');
    
    // Check if loadUserProfile is wrapped in useCallback
    if (authContent.includes('const loadUserProfile = useCallback')) {
      console.log('✅ useCustomerAuth: loadUserProfile properly memoized');
    } else {
      console.log('❌ useCustomerAuth: loadUserProfile not memoized');
      return false;
    }
    
    // Check if loadUserOrders is wrapped in useCallback
    if (ordersContent.includes('const loadUserOrders = useCallback')) {
      console.log('✅ useUserOrders: loadUserOrders properly memoized');
    } else {
      console.log('❌ useUserOrders: loadUserOrders not memoized');
      return false;
    }
